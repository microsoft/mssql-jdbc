...
/**
 * Utility class for generating random data for Always Encrypted testing
 */
public class RandomData {
	...
	public static BigDecimal generateDecimalNumeric(int precision, int scale, boolean nullable){
		...
		if(returnMinMax){
			...
			if(r.nextBoolean()){
				...
				if(scale>0)
					return new BigDecimal(n, scale).subtract(new BigDecimal(""+Math.pow(10, -scale)).setScale(scale, BigDecimal.ROUND_HALF_UP)).negate();
				else
	...
	public static Timestamp generateDatetime2(Integer precision, boolean nullable){
		...
		int precisionDigits = buildPrecision(precision, numberCharSet2); //not to use 0 in the random data for now. E.g creates 9040330 and when set it is 904033. 
		ts.setNanos(precisionDigits);
		return ts;
	}
	...
	private static BigDecimal generateMoneyOrSmallMoney(boolean nullable, BigDecimal max, BigDecimal min, float multiplier, String charSet){
		if(nullable){
			if(returnNull){
				return null;
			}
	...
	private static String buildCharOrNChar(String columnLength, boolean nullable, boolean encrypted, String charSet, int maxBound){
