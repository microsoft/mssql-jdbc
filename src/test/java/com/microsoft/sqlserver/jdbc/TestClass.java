...
public class TestClass {
    public static void main(String[] args) throws Exception {
        //sendStringParamsTest();
        sysUserNameTest();
    }

    public static void sysUserNameTest() throws Exception {
        Connection conn = DriverManager.getConnection("jdbc:sqlserver://jdbc-aas-sgx.westeurope.cloudapp.azure.com;databaseName=test;userName=sa;password=Moonshine4me;encrypt=false;trustServerCertificate=true");
        String targetUser = "jeffw";
        SQLServerDataSource ds = new SQLServerDataSource();
        ds.setClientKey();
        ds.setClientKeyPassword();
        ds.setClientCertificate();
        ds.setPassword();
        ds.setUser();


        try {
            System.out.println( "Before impersonate: " + conn.getMetaData().getUserName() );
            try( CallableStatement asOtherUser = conn.prepareCall( "EXECUTE AS USER = '" + targetUser + "'" ) ) {
                asOtherUser.execute();
                System.out.println( "After impersonate: " + conn.getMetaData().getUserName() );
            }

            try( CallableStatement revert = conn.prepareCall( "REVERT" ) ) {
                revert.execute();
                System.out.println( "After revert: " + conn.getMetaData().getUserName() );
            }
        }
        finally {
            conn.close();
    ...
    public static void sendStringParamsTest() throws Exception {
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver");
        // 获取连接
        try(Connection connection = DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=tempdb;userName=sa;password=TestPassword123;encrypt=false;trustServerCertificate=true")){
            // Sql
            ...
            while (resultSet.next()){
                ...
                if (iPutIndex >= 1000) {
                    ...
                    // 利用ID主键查询全量数据
                    try(Connection connection2 = DriverManager.getConnection("jdbc:sqlserver://localhost:1433;databaseName=tempdb;userName=sa;password=TestPassword123;encrypt=false;trustServerCertificate=true")){
                        // Sql
