...
public class DataTypesTest extends AbstractTest {
	...
	abstract static class SQLValue {
		...
		/*
		 * For testing the setObject and setNull methods in PreparedStatement, use the
		 * verifySetter* methods. These methods prepare a single statement and execute
		 * it for all different data types by calling the appropriate 'setObject'
		 * methods for each data type and/or type conversion.
		 */
	...
	static final class TimeValue extends SQLValue {
		...
		private void initExpected(String stringValue, TimeZone tz) {
			...
			this.nanos = (8 == stringValue.indexOf('.'))
					? (new BigDecimal(stringValue.substring(8))).scaleByPowerOfTen(9).intValue()
					: 0;
	...
	static final class DateTime2Value extends SQLValue {
		...
		private void initExpected(String timestampString, TimeZone tz) {
			...
			int nanos = (19 == timestampString.indexOf('.'))
					? (new BigDecimal(timestampString.substring(19))).scaleByPowerOfTen(9).intValue()
					: 0;
		...
		void verifyRSUpdaters(ResultSet rs) throws Exception {
			...
			/*
			 * Verify the update (after restoring the default time zone) using the
			 * getTimestamp variant that takes a time zone argument (as a Calendar)
			 */
			assertEquals(expectedTimestamp(), rs.getTimestamp(1, Calendar.getInstance(tz)));
		}
	...
	static final class DateTimeOffsetValue extends SQLValue {
		...
		void verifySettersUtilDate(PreparedStatement ps) throws Exception {
			/*
			 * Cannot test by setObject(...,java.util.Date) with the back end data type as
			 * datetimeoffset because by design anything but datetimeoffset is normalized to
			 * UTC time zone in the source code (in sendTemporal function in dtv.java)
			 * setObject(...,Date) and setObject(...,java.utl.date) gets same exception
			 */
		}
	...
	enum TestValue {
		...
		/*
		 * Dates in the Gregorian cutover date range appear as 10 days later than what
		 * they should. This behavior is consistent with other JDBC drivers, such as IBM
		 */
		...
		/*
		 * Post-cutover date requiring preservation of "wall calendar" date in computing
	...
	/*
	 * test CS.setObject(timestamp, TIME)/registerOutParam(TIME) with
	 * sendTimeAsDatetime
	 */
	@Test
		...
		try (Connection conn = PrepUtil.getConnection(connectionString + ";sendTimeAsDatetime=true")) {
			try (Statement stmt = conn.createStatement()) {
				...
				try (CallableStatement cs = conn.prepareCall("{call " + escapedProcName + "(?,?)}")) {
					...
					/*
					 * send the timestamp to the server using the TIME SQL type rather than
					 * TIMESTAMP. The driver will strip the date portion and, because
					 * sendTimeAsDatetime=true, round the resulting time value to midnight because
					 * it should be sending a DATETIME which has only 1/300s accuracy
					 */
	...
	/*
	 * test sending Timestamp to the server via an updater does not result in the
	 * same behavior as a setter wrt double-rounding of fractional seconds
	 */
	@Test
	@Tag(Constants.xAzureSQLDW)
	...
	/*
	 * Tests "fail fast" SQLException path when a Japanese imperial calendar is used
	 * with values representing the first year of an imperial era. See for more
	 * details:
	...
	public void testWithJapaneseImperialCalendar() throws Exception {
		/*
		 * From http://java.sun.com/javase/6/docs/api/java/util/Locale.html : "Note:
		 * When you ask for a resource for a particular locale, you get back the best
		 * available match, not necessarily precisely what you asked for. For more
		 * information, look at ResourceBundle." Japanese Imperial locale does not exist
		 * with some VMs. In these VMs, Locale.US is substituted as
		...
		try (Connection conn = PrepUtil.getConnection(connectionString + ";sendTimeAsDatetime=true")) {
			...
			try (PreparedStatement ps = conn.prepareStatement("SELECT CAST(? AS VARCHAR(40))")) {
				...
				/*
				 * Set second day of first year of Taisho era (1912 Gregorian) Note: Taisho era
				 * began July 30, 1912 Gregorian; second day of that year was July 31.
				 */
	...
	public void testWithThaiLocale() throws Exception {
		...
		try (Connection conn = PrepUtil.getConnection(connectionString + ";sendTimeAsDatetime=true")) {
			...
			try (PreparedStatement ps = conn.prepareStatement("SELECT CAST(? AS VARCHAR(40))")) {
				...
				/*
				 * Need to use the following constructor for running against IBM JVM. Here, year
				 * should be year-1900, // month // is from 0-11.
				 */
				Timestamp ts = new Timestamp(System.currentTimeMillis());

				/*
				 * Test PreparedStatement with Timestamp Value sent as DATETIME2; result should
				 * have 7 digits of subsecond precision)
				 */
				...
				/*
				 * Test PreparedStatement with Time Value sent as DATETIME w/Unix Epoch as base
				 * date when sendTimeAsDatetime=true
				 */
				...
				try (ResultSet rs = ps.executeQuery()) {
					...
					/*
					 * Timestamp returned from varchar column may be wrongly formatted. E.g: 12:12PM
					 * will not pass if compared to 12:12p.m.
					 */
	...
	/*
	 * test PS.setObject() on a datetime2 with values on or after 0700-02-29 have a
	 * value one day ahead stored in the server
	 */
	@Test
	...
	/*
	 * setTimestamp to DATETIMEOFFSET yields a value in local time with UTC time
	 * zone offset (+00:00) In this case, verify that SELECT with a WHERE clause
	 * does not fail due to mapping the Timestamp value to a SQL Server type that
	 * does not compare equal. For example, a DATETIMEOFFSET and DATETIME only
	 * compare equal if the DATETIMEOFFSET offset is 0 (UTC).
	 */
	@Test
	...
	/*
	 * SQL Server compatibility level >= 130 (SQL Server 2016 and after) doesn't
	 * round up or down equally for datetime. The rounding logic below represents
	 * the actual rounding logic employed by datetime columns when determining their
