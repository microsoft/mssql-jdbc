...
public class Main {
    ...
    private List<PerformanceTest> getTests() {
        return List.of(new PerfTablePerformanceTest(0)
    ...
    private void printResults(Map<String, PerformanceTest.PerformanceResult> testResults) {
        System.out.println();
        System.out.printf("T# %-75s%-15s%-13s%-15s%-15s%-12s%-6s%n", "Performance Test", "Result Count", "Time(ms)",
                "Size", "nullCount", "columnCount", "fsize");
        int index = 1;
        for (Map.Entry<String, PerformanceTest.PerformanceResult> entry : testResults.entrySet()) {
            ...
            System.out.printf("%2d %-75s%-15s%-13s%-15s%-15s%-12s%-6s%n", index, description, result.numberOfRecords(),
                    result.elapsedMilliseconds(), result.totalBytes(), result.nullColumnCount(), result.columnCount(),
                    result.rsFetchSize());
            index++;
        }
        System.out.println();
