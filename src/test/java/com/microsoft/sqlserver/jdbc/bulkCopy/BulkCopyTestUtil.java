...
class BulkCopyTestUtil {
	 ...
	 * @throws Exception
	 */
	static void validateValues(DBConnection con, ISQLServerBulkRecord srcData, DBTable destinationTable)
			throws Exception {
		...
		try (DBStatement dstStmt = con.createStatement();
			...
			// reset the counter in ISQLServerBulkRecord, which was incremented during read
			// by BulkCopy
			...
			while (srcData.next() && dstResultSet.next()) {
				...
				for (int i = 1; i <= totalColumns; i++) {
					...
					if (srcValue.getClass().getName().equalsIgnoreCase("java.lang.Double")) {
						// in case of SQL Server type Float (ie java type double), in float(n) if n is
						// <=24 ie precsion
					...
					if (java.sql.Types.TIMESTAMP != dstType && java.sql.Types.TIME != dstType
						...
						// skip validation for temporal types due to rounding eg 7986-10-21 09:51:15.114
						// is rounded as
