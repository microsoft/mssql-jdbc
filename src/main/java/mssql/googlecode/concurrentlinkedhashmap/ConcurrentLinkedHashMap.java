/*
 * Copyright 2010 Google Inc. All Rights Reserved. Licensed under the Apache License, Version 2.0 (the "License"); you
 ...
 * <code>Java Collections Framework</code>, this map does not have a publicly visible constructor and instances are
 * created through a {@link Builder}.
 * <p>
 * An entry is evicted from the map when the <code>weighted capacity</code> exceeds its
 * <code>maximum weighted capacity</code> threshold. A {@link EntryWeigher} determines how many units of capacity that
 * an entry consumes. The default weigher assigns each value a weight of <code>1</code> to bound the map by the total
 * number of key-value pairs. A map that holds collections may choose to weigh values by the number of elements in the
 * collection and bound the map by the total number of elements that it contains. A change to a value that modifies its
 * weight requires that an update operation is performed on the map.
 * <p>
 ...
 * <p>
 * The <code>concurrency level</code> determines the number of threads that can concurrently modify the table. Using a
 ...
 * <p>
 * Like {@link java.util.Hashtable} but unlike {@link HashMap}, this class does <em>not</em> allow <code>null</code> to
 * be used as a key or value. Unlike {@link java.util.LinkedHashMap}, this class does <em>not</em> provide predictable
...
public final class ConcurrentLinkedHashMap<K, V> extends AbstractMap<K, V>
     ...
     * were performed on the map are recorded in buffers. These buffers are drained at the first opportunity after a
     * write or when the read buffer exceeds a threshold size. The reads are recorded in a lossy buffer, allowing the
     * reordering operations to be discarded if the draining process cannot keep up. Due to the concurrent nature of the
     * read and write operations a strict policy ordering is not possible, but is observably strict when single
     * threaded. Due to a lack of a strict ordering guarantee, a task can be executed out-of-order, such as a removal
     * followed by its addition. The state of the entry is encoded within the value's weight. Alive: The entry is in
     * both the hash-table and the page replacement policy. This is represented by a positive weight. Retired: The entry
     * is not in the hash-table and is pending removal from the page replacement policy. This is represented by a
     * negative weight. Dead: The entry is not in the hash-table and is not in the page replacement policy. This is
     * represented by a weight of zero. The Least Recently Used page replacement algorithm was chosen due to its
    ...
    /**
     * Attempts to transition the node from the <code>alive</code> state to the <code>retired</code> state.
     *
     * @param node
     *        the entry in the page replacement policy
     * @param expect
    ...
    /**
     * Atomically transitions the node from the <code>alive</code> state to the <code>retired</code> state, if a valid
     * transition.
     *
     * @param node
     *        the entry in the page replacement policy
    ...
    /**
     * Atomically transitions the node to the <code>dead</code> state and decrements the <code>weightedSize</code>.
     *
     * @param node
     *        the entry in the page replacement policy
     */
     ...
     * {
     *     &#64;code
     *     ConcurrentMap<Vertex, Set<G>> graph = new Builder<Vertex, Set<G>>().maximumWeightedCapacity(5000)
     *             .weigher(Weighers.<G>set()).build();
     * }
     * </pre>
     */
    public static final class Builder<K, V> {
        ...
        /**
         * Specifies the initial capacity of the hash table (default <code>16</code>). This is the number of key-value
         * pairs that the hash table can hold before a resize operation is required.
         *
         ...
         * to try to accommodate this many threads (default <code>16</code>).
         *
         ...
         * bounds the map by the number of key-value pairs by giving each entry a weight of <code>1</code>.
         *
         * @param weigher
        ...
        public Builder<K, V> weigher(Weigher<? super V> weigher) {
            this.weigher = (weigher == Weighers.singleton()) ? Weighers
                    .<K, V>entrySingleton() : new BoundedEntryWeigher<K, V>(Weighers.asEntryWeigher(weigher));
            return this;
        }
