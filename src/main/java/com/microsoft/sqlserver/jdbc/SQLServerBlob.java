...
public final class SQLServerBlob extends SQLServerLob
		implements
			java.sql.Blob,
	...
	static private final Logger logger = Logger
			.getLogger("com.microsoft.sqlserver.jdbc.internals.SQLServerBlob");
	...
	SQLServerBlob(SQLServerConnection connection) {
		...
		if (logger.isLoggable(Level.FINE))
			logger.fine(
					toString() + " created by (" + connection.toString() + ")");
	}
	...
	private void checkClosed() throws SQLServerException {
		if (isClosed) {
			...
			SQLServerException.makeFromDriverError(con, null,
					form.format(new Object[]{"Blob"}), null, true);
		}
	}
		...
		if (null == value && !activeStreams.isEmpty()) {
			...
			if (value == null) {
				throw new SQLServerException(
						"Unexpected Error: blob value is null while all streams are closed.",
						null);
			}
	...
	public long position(java.sql.Blob pattern, long start)
		...
		return position(pattern.getBytes((long) 1, (int) pattern.length()),
				start);
	}
	...
	public long position(byte[] bPattern, long start) throws SQLException {
		...
		for (int pos = (int) start; pos <= value.length
				- bPattern.length; ++pos) {
			boolean match = true;
	...
	public java.io.OutputStream setBinaryStream(long pos) throws SQLException {
		...
		if (pos < 1) {
			MessageFormat form = new MessageFormat(
					SQLServerException.getErrString("R_invalidPositionIndex"));
			SQLServerException.makeFromDriverError(con, null,
					form.format(new Object[]{pos}), null, true);
		}
 ...
 * implementing class that forwards all calls to SQLServerBlob.setBytes. This
 * class is returned to caller by SQLServerBlob class when setBinaryStream is
 * called.
 * <p>
 * SQLServerBlobOutputStream starts writing at postion startPos and continues to
 * write in a forward only manner. Reset/mark are not supported.
 */
