...
public class SQLServerConnection implements ISQLServerConnection, java.io.Serializable {
    ...
    public boolean isValid(int timeout) throws SQLException {
        ...
        try {
            ...
            isValid = false;
            /*
             * Do not propagate exceptions from query execution or statement closure. The
             * connection is considered to be
             * invalid if the statement fails to close, even though query execution
             * succeeded.
             */
            if (connectionlogger.isLoggable(Level.FINE)) {
            ...
            if (networkTimeoutChanged) {
                try {
                    ...
                    isValid = false;
                    if (connectionlogger.isLoggable(Level.FINE)) {
                        connectionlogger.fine(toString() + " Exception restoring network timeout: " + e.getMessage());
                    }
                }
            }
        }
