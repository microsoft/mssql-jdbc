 ...
 * creating a JDBC connection to SQL Server. SQLServerConnections support JDBC
 * connection pooling and may be either physical JDBC connections or logical
 * JDBC connections.
 *
 ...
 * SQLServerConnectionPoolProxy Declare all new custom (non-static) Public APIs
 * in ISQLServerConnection interface such that they can also be implemented by
...
public class SQLServerConnection implements ISQLServerConnection, java.io.Serializable {
	 ...
	 * sp_executesql -> sp_prepexec -> sp_execute -> batched -> sp_unprepare
	 * pattern, true == skip sp_executesql part of pattern.
	 */
	 ...
	 * 
	 * @param sql SQL text to parse for positions of parameters to initialize.
	 */
	...
	synchronized SQLServerColumnEncryptionKeyStoreProvider getGlobalSystemColumnEncryptionKeyStoreProvider(
		...
		return (null != globalSystemColumnEncryptionKeyStoreProviders
				&& globalSystemColumnEncryptionKeyStoreProviders.containsKey(providerName))
						? globalSystemColumnEncryptionKeyStoreProviders.get(providerName)
						: null;
	}
	...
	synchronized String getAllGlobalCustomSystemColumnEncryptionKeyStoreProviders() {
		return (null != globalCustomColumnEncryptionKeyStoreProviders)
				? globalCustomColumnEncryptionKeyStoreProviders.keySet().toString()
				: null;
	}
	...
	synchronized SQLServerColumnEncryptionKeyStoreProvider getGlobalCustomColumnEncryptionKeyStoreProvider(
		...
		return (null != globalCustomColumnEncryptionKeyStoreProviders
				&& globalCustomColumnEncryptionKeyStoreProviders.containsKey(providerName))
						? globalCustomColumnEncryptionKeyStoreProviders.get(providerName)
						: null;
	}

	synchronized SQLServerColumnEncryptionKeyStoreProvider getSystemColumnEncryptionKeyStoreProvider(
			String providerName) {
		return (null != systemColumnEncryptionKeyStoreProvider
				&& systemColumnEncryptionKeyStoreProvider.containsKey(providerName))
						? systemColumnEncryptionKeyStoreProvider.get(providerName)
						: null;
	}

	synchronized SQLServerColumnEncryptionKeyStoreProvider getColumnEncryptionKeyStoreProvider(String providerName)
			throws SQLServerException {

		// Check for the connection provider first.
		keystoreProvider = getSystemColumnEncryptionKeyStoreProvider(providerName);

		// There is no connection provider of this name, check for the global system
		// providers.
	 ...
	 * flag's value is reset to false when a new transaction starts or when the
	 ...
	 * session is not available before the session is established and after the
	 * session is closed.
	 */
	 ...
	 * resetting the values after the connection is released to the pool for reuse.
	 */
	 ...
	 * failover map, the connecting server returned the following variable only
	 * stores the serverReturned failver information.
	 */
	 ...
	 * packetSize connection property. The value was chosen to take maximum
	 ...
	 * append the ClientConnectionId to error string.
	 */
	 ...
	 * stmt.getConnection, these functions should return the proxy not the actual
	 * physical connection when the physical connection is pooled and the user
	 ...
	 * instance name, application name, etc...). See TDS specification, "Login Data
	...
	Connection connectInternal(Properties propsIn, SQLServerPooledConnection pooledConnection)
		...
		try {
			 ...
			 * maintain this value for every instance of SQLServerConnection.
			 */
			...
			sendStringParametersAsUnicode = (null == activeConnectionProperties.getProperty(sPropKey))
			...
			// DBMirroring is true and user did not
			// set TNIR explicitly
	 ...
	 * standard connection the server can provide us with its FO partner. If no FO
	 * information is available a standard connection is made. If the server returns
	 * a failover upon connection, we shall store the FO in our cache.
	 */
	 ...
	 * failoverPermissionCheck Only difference is that this gets the instance port
	 * if the port number is zero where as failover does not have port number
	 * available.
	 */
	...
	boolean executeCommand(TDSCommand newCommand) throws SQLServerException {
		synchronized (schedulerLock) {
			 ...
			 * can execute the new command. Note that detaching the response does not
			 * process it. Detaching just buffers the response off of the wire to clear the
			 * TDS channel.
			 */
			if (null != currentCommand) {
				try {
					 ...
					 * Our purpose to detach the response and empty buffer is done here. If there is
					 * anything wrong with the connection itself, let the exception pass below to be
					 * thrown during 'execute()'.
					 */
					if (connectionlogger.isLoggable(Level.FINE)) {
			 ...
			 * command at a time may use a connection (to avoid TDS protocol errors), just
			 * synchronize to serialize command execution.
			 */
			...
			try {
				 ...
				 * ResultSet or complex response with multiple results) then remember it as the
				 * current command so that any subsequent call to executeCommand will detach it
				 * before executing another new command.
				 */
	...
	public void abort(Executor executor) throws SQLException {
		...
		if (null == executor) {
			 ...
			 * happens when we try to clean up the physical resources associated with the
			 * connection using executor.
			 */
	 ...
	 * connection proxy is closed This event will pool the connection
	 */
	...
	int writeFedAuthFeatureRequest(boolean write, /* if false just calculates the length */
			TDSWriter tdsWriter, FederatedAuthenticationFeatureExtensionData fedAuthFeatureExtensionData)
			throws SQLServerException {
	...
	private void logon(LogonCommand command) throws SQLServerException {
		...
		// have to insert FedAuth Feature
		// Extension
	...
	private SqlFedAuthToken getMSIAuthToken(String resource, String msiClientId) throws SQLServerException {
		...
		/*
		 * isAzureFunction is used for identifying if the current client application is
		 * running in a Virtual Machine (without MSI environment variables) or App
		 * Service/Function (with MSI environment variables) as the APIs to be called
		 * for acquiring MSI Token are different for both cases.
		 */
		...
		if (isAzureFunction) {
			...
			// Retry acquiring access token upto 20 times due to possible IMDS upgrade
			// (Applies to VM only)
			maxRetry = 20;
	 ...
	 * 
	 * @param logonCommand                the logon command
	 * @param authentication              SSPI authentication
	 * @param fedAuthFeatureExtensionData fedauth feature extension data
	 * @throws SQLServerException
	 */
	...
	public boolean isValid(int timeout) throws SQLException {
		...
		try (SQLServerStatement stmt = new SQLServerStatement(this, ResultSet.TYPE_FORWARD_ONLY,
			 ...
			 * it fails (throws an exception), then the connection is not valid. If a
			 * timeout was provided, execution throws an "query timed out" exception if the
			 * query fails to execute in that time.
			 */
			 ...
			 * connection is considered to be invalid if the statement fails to close, even
			 * though query execution succeeded.
			 */
	...
	/**
	 * Replaces JDBC syntax parameter markets '?' with SQL Server parameter
	 * markers @p1, @p2 etc...
	 * 
	 ...
	 * activity on the connection. Calling notifyPooledConnection with null event
	 * will place this connection back in the pool. Calling notifyPooledConnection
	 * with a non-null event is used to notify the pooling manager that the
	 * connection is bad and should be removed from the pool.
	 */
	 ...
	 * Azure. It should return 6 for SQL Azure DW. This is more reliable
