...
final class VectorInputStream extends BaseInputStream {

    VectorInputStream(TDSReader tdsReader, int payLoadLength, InputStreamGetterArgs getterArgs, ServerDTVImpl dtv) {
        super(tdsReader, getterArgs.isAdaptive, getterArgs.isStreaming, dtv);
        setLoggingInfo(getterArgs.logContext);
        this.payloadLength = payLoadLength;
    }
    ...
    final byte[] getBytes() throws SQLServerException {
        ...
        if (payloadLength <= 8) {
            throw new SQLServerException("Invalid vector data: less than 8 bytes.", null);
        }

        byte[] value = new byte[payloadLength - 8]; // Allocate only the required bytes

        try {
            skip(8); // Skip first 8 bytes (header)
            read(value); // Read remaining bytes into value
            close();
        } catch (IOException e) {
            SQLServerException.makeFromDriverError(null, null, e.getMessage(), null, true);
        }
