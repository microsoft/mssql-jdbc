...
**Who benefits**: Applications querying `varchar`/`char` columns - especially **partitioned** tables - whose execution plans degrade because the parameter is mis-declared. Two distinct plan problems are addressed: (1) under the default `sendStringParametersAsUnicode=true` an `nvarchar` parameter forces an implicit convert on a `varchar`/`char` column, turning an Index Seek into a Scan; (2) even after switching to `varchar` (`sendStringParametersAsUnicode=false`), the fixed `varchar(8000)` width still **defeats partition elimination** - the plan seeks but probes every partition - because the partition function's column type is narrower. Sizing the parameter to the column's actual width (e.g. `varchar(8)`) is what lets the optimizer eliminate to the single matching partition. Right-sized memory grants (fewer `RESOURCE_SEMAPHORE` waits / tempdb spills) are an additional benefit.
**Measured (SQL Server 2022, `varchar(8)`-partitioned table)**: `nvarchar(4000)` -> Scan, all 13 partitions; `varchar(8000)` -> Seek but still all 13 partitions; `varchar(8)` (this feature) -> Seek, **1** partition. On a non-partitioned `varchar(17)` index, `nvarchar(4000)` did 1548 logical reads while both `varchar(8000)` and `varchar(17)` did 3 - so on a plain index `sendStringParametersAsUnicode=false` alone restores the seek and the column-sized length only tightens the memory grant; the length becomes decisive for partition elimination.
**Impact**: Opt-in and backward compatible - default behavior is byte-for-byte unchanged. Combine with `sendStringParametersAsUnicode=false` so `varchar`/`char` parameters are emitted as `varchar` (required for the seek/elimination on those columns); the feature then sizes them to the column. Falls back cleanly to the legacy default for range/expression/ambiguous parameters, Always Encrypted connections, the FMTONLY path, and pre-2012 servers. Relates to [#2960](https://github.com/microsoft/mssql-jdbc/pull/2960).
