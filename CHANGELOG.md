...
- **Fix Silent Drop of Trailing SQL After VALUES in Bulk Copy Batch Insert** [#3037](https://github.com/microsoft/mssql-jdbc/pull/3037)
  ...
  - **Impact**: The full statement is preserved on every batch, including repeated `executeBatch()` and `executeLargeBatch()` calls on the same prepared statement. Unsupported trailing SQL is checked once and keeps using the regular batch path; statements with only trailing comments, whitespace, or semicolons continue to use Bulk Copy.
